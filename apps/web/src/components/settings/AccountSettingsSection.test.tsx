...
function AccountTree({ runtime }: { runtime: OsirisRuntimeContextValue }) {
  return (
    <TestI18nProvider>
...
async function renderAccount(runtime: OsirisRuntimeContextValue = baseRuntime) {
  const container = document.createElement('div');
  document.body.append(container);
  const root = createRoot(container);

  await act(async () => {
    root.render(<AccountTree runtime={runtime} />);
  });
